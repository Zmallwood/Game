...
namespace Zmallwood {
    ...
    void RendererBase::SetIndicesData(GLuint indicesVBOID, int numVertices,
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                     numVertices *
                         k_numFloatsPerEntry.at(BufferTypes::Indices) *
                         sizeof(float),
    ...
    void RendererBase::SetArrayBufferDataInt(GLuint VBOID, int numVertices,
        ...
        glBindBuffer(GL_ARRAY_BUFFER, VBOID);
        glBufferData(GL_ARRAY_BUFFER,
                     numVertices * numFloatsPerEntry * sizeof(int), data,
                     GL_DYNAMIC_DRAW);
    ...
    void RendererBase::UpdateIndicesData(GLuint indicesVBOID,
        ...
        glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0,
                        sizeof(float) * indices.size(), indices.data());
    }
    ...
    void RendererBase::UpdateArrayBufferDataInt(GLuint VBOID,
                                                ...
                                                int layoutLocation) const {
        glBindBuffer(GL_ARRAY_BUFFER, VBOID);
        glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(int) * data.size(),
                        data.data());
        glVertexAttribIPointer(layoutLocation, numFloatsPerEntry, GL_INT, 0,
                               (const GLvoid *)0);
        glEnableVertexAttribArray(layoutLocation);
