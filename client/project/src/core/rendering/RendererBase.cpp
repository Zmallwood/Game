...
namespace Zmallwood {
  ...
  void RendererBase::SetData(GLuint VBOID, int numVertices, const void *data,
    ...
    if (buffType == BufferTypes::BoneIDs) {
      SetArrayBufferDataInt(VBOID, numVertices, data,
  ...
  void RendererBase::SetArrayBufferData(GLuint VBOID, int numVertices,
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 numVertices * numFloatsPerEntry * sizeof(float), data,
                 GL_DYNAMIC_DRAW);

    if (layoutLocation >= 0) {
      glVertexAttribPointer(layoutLocation, numFloatsPerEntry, GL_FLOAT,
  ...
  void RendererBase::SetArrayBufferDataInt(GLuint VBOID, int numVertices,
                                           ...
                                           int layoutLocation) const {
    glBindBuffer(GL_ARRAY_BUFFER, VBOID);
    glBufferData(GL_ARRAY_BUFFER, numVertices * numFloatsPerEntry * sizeof(int),
                 data, GL_DYNAMIC_DRAW);

    if (layoutLocation >= 0) {
      glEnableVertexAttribArray(3);
      glVertexAttribIPointer(layoutLocation, numFloatsPerEntry, GL_INT, 0,
                             (const GLvoid *)0);
      glEnableVertexAttribArray(layoutLocation);
    }
  }
  ...
  void RendererBase::UpdateIndicesData(GLuint indicesVBOID,
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indicesVBOID);
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, sizeof(float) * indices.size(),
                    indices.data());
  }
  ...
  void RendererBase::UpdateData(GLuint VBOID, std::vector<float> &data,
                                ...
                                int layoutLocation) const {
    if (buffType == BufferTypes::BoneIDs) {
      UpdateArrayBufferDataInt(VBOID, data,
                               RendererBase::k_numFloatsPerEntry.at(buffType),
                               layoutLocation);
    } else {
      UpdateArrayBufferData(VBOID, data,
                            RendererBase::k_numFloatsPerEntry.at(buffType),
                            layoutLocation);
    }
  }
